...
- ⬜ Next: Slice 3 Phase B — R_01–R_11 deterministic Critic (C10–C14 per slice-3-runbook.md Steps 2–6). R_11 becomes the durable regression insurance on top of Step 0's prompt fix; R_01–R_10 add structural-integrity gates (hallucination, path-consistency, scope, tool-match, etc.) the current pipeline doesn't yet enforce programmatically.
...
**Recommended next:**
1. **Slice 3 Phase C** (~1 focused day): items 14 L3 primitives (plan-hash dedup + debounce hook + thread-scoped checkpointer) + R_06 Negative-Result-Metadata + R_12 Evidence-of-Absence + R_13 Temporal Consistency + node-wiring close-out (C6/C9 bodies into `plan_node`/`interpret_node`, idempotency-guard relaxation). Closes Slice 3 end-to-end.
2. **Slice 5** per the drafted runbook: dual-channel handler + capability tokens + 5th MCP tool (`scheduled_tasks_parse`). Depends on Phase C rules landing first so the structured-field metadata has real consumers.
