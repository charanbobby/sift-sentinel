...
**Companion to:** [vision.md](vision.md) (philosophy), [PLAN.md](PLAN.md) (slices + status)
**Audience:** hackathon judges + future maintainers.

This document is the submission-quality architectural reference. It explains the pipeline, trust boundaries, state schema, tool surface, and Critic catalog in enough detail that a reviewer can trace any finding back to its producing component. Where concrete implementation lives in a specific notebook cell or module, we say so by name — this is an engineering document, not a positioning one.
...
| `tool_plan_hash` | `str \| None` | `PLAN` → `CRITIC` | `dedup` | SHA-256 of the plan — drives L3 dedup primitive |
