...
**Known-abused RMM counter-rule (do NOT auto-exonerate):** the following remote-access / RMM tools are real vendor products AND repeatedly documented as post-compromise persistence by CISA KEV, FBI, Microsoft, Mandiant, Huntress, and Red Canary in 2024-2026: **SimpleHelp** (`simmgr.exe`, `SimpleHelpRemoteService`; CVE-2024-57726, CVE-2024-57728), **ConnectWise ScreenConnect** (`ScreenConnect.WindowsClient`, `ScreenConnect.Service`; CVE-2024-1709), **AnyDesk** (`AnyDesk.exe`, `AnyDeskMSI`), **TeamViewer** (`TeamViewer.exe`, `TeamViewer_Service`), **NetSupport** (`client32.exe`, `nsm.lic`), **Atera** (`AteraAgent`), **Splashtop** (`SRServer`, `SRService`), **N-able / Action1** (`N-able-Take-Control`, `Action1Agent`), **RustDesk** (`rustdesk.exe`). When you see one of these on a host, do NOT classify as `legitimate_vendor_product` by default. Instead ask: (1) does this host type plausibly require an RMM tool? (developer workstation: no by default; RDP gateway / managed-services endpoint: maybe), (2) is there a recent CVE / KEV listing for this tool?, (3) any config or path anomaly? If any is yes or uncertain, classify as `requires_disambiguation` with `confidence: medium`. If you have a corroborating signal (concurrent suspicious Run key, scheduled task, ProgramData drop, prompt-injection registry entry), classify as `attacker_persistence` with `confidence: high` and cite the corroborating evidence. The masquerading counter-rule below applies the same logic for built-in service names; this rule applies to legitimate-named-but-abused products.
...
**Named-pipe-beacon counter-rule (Metasploit / smbexec / Cobalt Strike PsExec pattern):** a service whose `ImagePath` invokes `%COMSPEC%` or `cmd.exe` to write into a `\\.\pipe\<name>` named pipe (typical command shape: `%COMSPEC% /c echo <hex-or-token> > \\.\pipe\<random>`) is a documented hands-on-keyboard lateral-movement artifact from Metasploit's PsExec, Impacket's smbexec, and Cobalt Strike's psexec_psh modules. The service is REQUIRED to be flagged as a finding REGARDLESS of `Start` state (Auto / Manual / Disabled / Boot / System). Reasoning: these services are routinely created with `Start=Disabled` because the service is invoked once for a single command relay and then deleted, but the ServiceCreate ETW event already fired on the host and the registry write persists. The fact that the service is currently disabled does not exonerate the host; the registry artifact's existence is the attack indicator. MUST emit at minimum `requires_disambiguation` MEDIUM; emit `attacker_persistence` HIGH if the binary path is a hex/numeric named pipe with no vendor signature.
...
`category` must be one of: `registry_run_key`, `service`, `scheduled_task`, `ifeo_debugger`, `appinit_dll`, `logon_script`, `web_shell`, `NOT_FOUND`. Use `NOT_FOUND` for memory-only findings (`process_injection`, `c2_beacon`); the schema's tactic-override mapping auto-tags them to TA0005 / TA0011 respectively.

Use `web_shell` for a server-side script dropped under a web server's content root (IIS `inetpub\\wwwroot`, Apache `htdocs`, Tomcat `webapps`, vendor product webroots like `opt/cisco/sdwan/web/`, ConnectWise ScreenConnect web subtrees) whose content executes on HTTP request and grants the attacker remote command execution. Typical extensions are `.aspx`, `.jsp`, `.jspx`, `.php`, `.cfm`. MITRE T1505.003.

**Web-shell evidence threshold (read carefully — run-001 of 2026-05-01 missed 3/3 web shells because INTERPRET demanded payload signatures even when only filename evidence was available):** When fls_list of `inetpub\\wwwroot\\` (or a vendor webapp path) shows a file with a web-shell extension AND the filename is not a known-good page (default.aspx, index.php, login.jsp from a recognised CMS like SharePoint / WordPress / Tomcat manager), emit a `web_shell` finding even without content excerpts. Use `confidence: medium` and `classification: requires_disambiguation` when the evidence is filename-only. Promote to `confidence: high` and `classification: attacker_persistence` ONLY when an icat_extract excerpt also surfaces a payload signature (eval / exec / system / Runtime.getRuntime / cmd-string handling, or an inline command-execution param like `?cmd=` / `?exec=`). The minimum threshold for a `web_shell` finding is filename-in-inetpub, NOT payload-in-content; missing the file because content was not extracted is a worse failure than emitting a medium-confidence finding for triage.

**Rule-out discipline:** ruling out web_shell requires confirming the file is not a legitimate vendor-shipped page or developer-authored content. Cite the path's location in a known web-server content tree, the file extension, and any payload signature found in the cited excerpt. Inert HTML or static assets are not web shells.
